...
import ProjectImage from "../../assets/ProjectImage.astro";
import logo from "../../assets/images/projects/zx80/sinclair_zx80_logo.svg";
import schematic from "../../assets/images/projects/zx80/ZX80_Schematic.png"
import board from "../../assets/images/projects/zx80/ZX80_Board.png"
import key from "../../assets/images/projects/zx80/ZX80_Key.png"

<Link href="https://github.com/TankedThomas/ZX80" aria-label="ZX80 Replica GitHub Repo" target="_blank">
    <ProjectImage src={logo} alt="Sinclair ZX80 Logo" loading="eager" />
</Link><br />
...
<Link href="https://raw.githubusercontent.com/TankedThomas/ZX80/master/CHANGELOG.md" aria-label="Changelog">Changelog</Link> (Last updated 07/03/2025)  

<Link href="#project-goals" aria-label="Project Goals">Project Goals</Link>  
<Link href="#future-goals" aria-label="Future Goals">Future Goals</Link>  
<Link href="#software" aria-label="Software">Software</Link>  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<Link href="#previous-work" aria-label="Previous Work">Previous Work</Link>  
<Link href="#project-status" aria-label="Project Status">Project Status</Link>  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<Link href="#schematics" aria-label="Schematics">Schematics</Link>  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<Link href="#pcb" aria-label="PCB">PCB</Link>  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<Link href="#parts-list" aria-label="Parts List">Parts List</Link>  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<Link href="#case" aria-label="Case">Case</Link>  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<Link href="#rf-modulator" aria-label="RF Modulator">RF Modulator</Link>  
<Link href="#current-issues" aria-label="Current Issues">Current Issues</Link>  
<Link href="#contributing" aria-label="Contributing">Contributing</Link>  
<Link href="#licensing" aria-label="Licensing">Licensing</Link>  

## Project Goals

My goal is to recreate the original ZX80 schematic and PCB as accurately as possible in modern software so that replicas can be made via PCB manufacturers such as <Link href="https://jlcpcb.com" aria-label="JLCPCB">JLCPCB</Link> and <Link href="https://www.pcbway.com" aria-label="PCBWay">PCBWay</Link>.  
...
Hopefully, the schematics can be submitted to the awesome <Link href="https://github.com/baldengineer/bit-preserve/" aria-label="Bit Preserve">Bit Preserve</Link> project in the future - a project I recently found that has similar albeit less ambitious goals than my own (primarily that it isn't worried about also replicating the PCBs, just the schematics).  
...
All my work has been undertaken using <Link href="https://www.kicad.org/" aria-label="KiCad">KiCad</Link>.  

#### Previous Work

The original schematic is &copy;Sinclair Research.  
<Link href="http://searle.x10host.com/zx80/zx80.html" aria-label="Grant Searle's redrawn schematic">Grant Searle's redrawn schematic</Link> is the basis for this project, and mostly corrects the issues with the original schematic.  
...
Instead, I used <Link href="https://www.8bity.cz/zx80-replika/navod-na-stavbu-repliky-zx80/" aria-label="Martin's modified PCB foils">Martin's modified PCB foils</Link> as a template for placing and aligning parts on the PCB.  

Martin mentioned two corrections he made ~~but does not elaborate, so I will have to look into this at a future date.~~ These corrections are mentioned in a <Link href="https://www.8bity.cz/zx80-replika/jak-jsem-stavel-repliku-zx80/" aria-label="different article">different article</Link> on his website, about halfway down the page.  
...
I have also included relatively high quality scans of the original <Link href="https://github.com/TankedThomas/ZX80/blob/master/Docs/ZX80_Assembly_Part1.jpg" aria-label="assembly">assembly</Link> <Link href="https://github.com/TankedThomas/ZX80/blob/master/Docs/ZX80_Assembly_Part2.jpg" aria-label="guide">guide</Link> (will add the source when I find it again), which has some contradictions to the oft-seen "original" schematic (including the one Grant provides), such as `C12` being `100nF` instead of `47nF`, and the ROM being a `2332` instead of an `8332`, and I have chosen to use the differences from this version instead as it appears to make more sense to me.  

Finding high-resolution photos of the original PCB is quite difficult, but I have <Link href="https://github.com/TankedThomas/ZX80/blob/master/Docs/ZX80Iss2.jpg" aria-label="included one that Grant supplied">included one that Grant supplied</Link> as it has been invaluable (albeit imperfect) to me so far.  
...
First is <Link href="https://blog.qiqitori.com/2024/03/building-a-new-zx80/" aria-label="Qiqitori">Qiqitori,</Link> who has taken a different approach to recreating the PCB than me but has a finished product. This is pretty good if you want a mostly accurate replica now, though I'm not 100% happy with it (we'll say 99% - I'm a perfectionist).  
Then there is <Link href="https://ekaww4.wixsite.com/tecnologia/post/projeto-5-sinclair-zx80" aria-label="Emilio Aoi">Emilio Aoi,</Link> who also has a finished board that's a fairly close replica, but he took some liberties with the keyboard and the video circuit. If you're after a ZX80 clone that'll work on a modern display and not feel like you're typing on a plank of wood, you can even get his PCBs directly from <Link href="https://www.pcbway.com/project/shareproject/ZX80_SINCLAIR_REPLICA_066e073d.html" aria-label="PCBWay">PCBWay.</Link>  
...
Currently, <Link href="https://github.com/TankedThomas/ZX80/blob/master/Docs/zx80_schematic_kicad.pdf" aria-label="the schematic">the schematic</Link> is more or less complete though not visually identical to the original.  

<ProjectImage src={schematic} alt="Schematic" /><br />
...
<ProjectImage src={board} alt="WIP PCB" /><br />
...
<ProjectImage src={key} alt="Recreated ZX80 Keypad" /><br />

#### Parts List

A <Link href="../../assets/files/projects/zx80/zx80_parts_list.csv" aria-label="parts list is available as a CSV">parts list is available as a CSV</Link>, though it is not completed yet (primarily the capacitors and the RAM/ROM ICs).  
...
A 3D-printable case is available, with <Link href="https://www.printables.com/model/490727-sinclair-zx80-replica-case" aria-label="Spinetti's version">Spinetti's version</Link> appearing to be the best so far.  
...
See <Link href="../../assets/files/projects/zx80/ERC.rpt" aria-label="ERC.rpt">ERC.rpt</Link> for current schematic wiring issues as reported by KiCad's Electrical Rule Checker.  
...
<Link href="https://www.8bity.cz/zx80-replika/navod-na-stavbu-repliky-zx80/" aria-label="Martin">Martin</Link> suggests that the original ROMs were on an 8332 or TMS4732, with the D2364C indeed used for the ZX80 to ZX81 upgrade.  
On the other hand, <Link href="http://searle.x10host.com/zx80/zx80.html#Using%20Alternative%20Components" aria-label="Grant">Grant</Link> says his original ZX80 used a 2532 with a snipped leg connected to VCC - was this how the EPROM was originally supplied, or is this a replacement/modification made after the fact?  
